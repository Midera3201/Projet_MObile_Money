...
class ClientController extends BaseController
{
    ...
    private function calculerFrais($typeOperation, $montant)
    {
        $db = \Config\Database::connect();
        $bareme = $db->query(
            "SELECT frais_fixe, frais_pourcentage FROM baremes b
             JOIN types_operations t ON b.id_type_operation = t.id
             WHERE t.code = ? AND ? BETWEEN b.montant_min AND b.montant_max",
    ...
    public function storeRetrait()
    {
        ...
        $client = $this->clientModel->find($this->currentUser['id']);
        if ($client['solde'] < $total) return redirect()->back()->with('error', 'Solde insuffisant. Solde: ' . number_format($client['solde'], 0, ',', ' ') . ' FCFA, Total requis: ' . number_format($total, 0, ',', ' ') . ' FCFA.');
